...
const userController = {
    register: async (req, res) => {
        try {
            ...
            res.status(201).json({ message: "User created successfully", data: response, token });
        } catch (error) {
    ...
    login: async (req, res) => {
        try {
            ...
            if( userExists && (await userExists.matchPassword(password))){
                const token = await generateToken(res, userExists._id);
                return res.status(200).json({ message: "User authenticated successfully", data: userExists, token })
            }else{
